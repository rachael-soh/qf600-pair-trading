...
for each industry cohort.

Reads:
  - Panel: crsp_daily_1961_1973.csv  (includes ret, dlret, ret_total, adj_prc)
  - Cohorts: industry_monthly_cohorts_top20/matched_pairs_monthly_YYYYMM.csv (with industry column)
  - French monthly factors: F-F_Research_Data_Factors.csv, F-F_Momentum_Factor.csv, F-F_ST_Reversal_Factor.csv

Outputs -> industry_monthly_trading_ffmr_top20_10y/ :
  - <industry>/portfolio_daily.csv
  - <industry>/portfolio_monthly.csv
  - <industry>/ffmr_regression_table.csv
  - <industry>/ffmr_performance_stats.csv
"""
...
COHORT_DIR = "industry_monthly_cohorts_top20"
TRADING_BDAYS = 126
WAIT_ONE_DAY = False
TOP_N_CAP = 9999
Z_ENTRY = 2.0
...
def compute_mu_sd_from_formation(panel, i, j, p):
    ...
    si = panel[(panel["permno"]==i) & (panel["date"].between(f_start,f_end))].sort_values("date")["adj_prc"]
    sj = panel[(panel["permno"]==j) & (panel["date"].between(f_start,f_end))].sort_values("date")["adj_prc"]
...
def build_portfolio_series_by_industry():
    panel = load_panel()
    files = sorted(glob.glob(os.path.join(COHORT_DIR, "matched_pairs_monthly_*.csv")))
    if not files:
        raise RuntimeError("No cohort files found in industry_monthly_cohorts_top20/.")

    industry_pair_series = defaultdict(list)
    for path in files:
        ...
        if "permno_1" not in df.columns or "industry" not in df.columns:
            continue
        ...
        for _, row in df.iterrows():
            ...
            industry_pair_series[industry].append(series)

    if not industry_pair_series:
        raise RuntimeError("No industry pair-level series built.")

    # For each industry, build daily and monthly portfolio
    industry_portfolios = {}
    for industry, pair_series in industry_pair_series.items():
        ...
        industry_portfolios[industry] = (portfolio_daily, portfolio_monthly)
...
def run_ffmr(portfolio_monthly, outdir, industry):
    ...
    for name, label in [("const","Intercept (Alpha)"),("Mkt-RF","Mkt-RF"),("SMB","SMB"),
        ...
        rows.append({"Model":f"FFMR Monthly Cohorts (Pair-level EW, {industry})","Factor":label,
    ...
    perf = pd.DataFrame({
        "Series":[f"FFMR Monthly Cohorts (Pair-level EW, {industry})"],
...
def main():
    ...
    if WAIT_ONE_DAY:
        OUTDIR = f"industry_monthly_trading_ffmr_{topN}_wait1d"
    else:
        OUTDIR = f"industry_monthly_trading_ffmr_{topN}"

    os.makedirs(OUTDIR, exist_ok=True)
    industry_portfolios = build_portfolio_series_by_industry()
    for industry, (daily, monthly) in industry_portfolios.items():
        safe_industry = industry.replace("/", "-").replace(" ", "_")
        outdir = os.path.join(OUTDIR, safe_industry)
        os.makedirs(outdir, exist_ok=True)
        daily.to_csv(os.path.join(outdir, "portfolio_daily.csv"), header=["r_p_daily"])
        run_ffmr(monthly, outdir, industry)
        print(f"Done. Outputs for {industry} in {outdir}")
