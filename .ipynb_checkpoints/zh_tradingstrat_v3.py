...
def export_monthly_returns(results, output_file='monthly_returns.csv'):
    ...
    for r in results:
        ...
        if trades and isinstance(spread, pd.Series) and not spread.empty:
            ...
            for m in months:
                ...
                if open_trade:
                    # Ensure spread index is datetime
                    if not isinstance(spread.index, pd.DatetimeIndex):
                        spread.index = pd.to_datetime(spread.index)

                    # Safely get last available spread
                    ...
                    monthly_returns.loc[month_end] += unrealized_pnl
                            # Ensure index alignment before adding
    ...
    if pair_monthly_returns:
        ...
        all_cumulative_returns = all_monthly_returns.cumsum()
...
if __name__ == "__main__":
    # Connect to WRDS
    db = wrds.Connection(wrds_username='erhzhihong')
