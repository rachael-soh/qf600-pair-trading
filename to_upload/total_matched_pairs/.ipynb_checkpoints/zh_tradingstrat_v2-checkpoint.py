...
def export_monthly_returns(results, output_file='monthly_returns.csv'):
    ...
    Each pair's result in `results` should contain:
        - 'pair': str, the pair name
        - 'spread_series': pd.Series, indexed by date
        - 'trades': list of trade dicts with 'diverge_date', 'converge_date', 'entry_spread', 'position', 'pnl'
    
    Args:
        results (list): List of result dicts (output from simulate_trading)
        output_file (str): CSV filename to save combined monthly returns
    
    Returns:
        pd.DataFrame: Combined monthly returns for all pairs
    """
    ...
    for r in results:
        ...
        if trades and isinstance(spread, pd.Series) and not spread.empty:
            ...
            # --- Full monthly timeline (no blanks) ---
            months = pd.period_range(spread.index.min().to_period('M'),
                                     spread.index.max().to_period('M'))
            monthly_returns = pd.Series(0.0, index=months.to_timestamp())
            ...
            for m in months:
                ...
                if open_trade:
                    # Use the last available spread of that month
                    month_spread = spread.loc[:month_end].iloc[-1]
            ...
            # Store this pair's monthly returns
            monthly_returns.name = pair_name
            pair_monthly_returns.append(monthly_returns)
        
        else:
            print(f"⚠️ Skipping pair {pair_name}: No trades or missing spread data")
    ...
    if pair_monthly_returns:
        ...
        print(f"\n✅ Exported combined monthly returns to: {output_file}")
...
if __name__ == "__main__":
    ...
    data = fetch_crsp_data(db, '2023-01-01', '2023-12-31')
    print(f"\nCRSP data fetched: {data.shape}")
    print(data.head())

    # Get trading data (next 6 months after formation)
    trading_data = fetch_crsp_data(db, '2024-01-01', '2024-12-31')
    ...
    export_monthly_returns(results, output_file='monthly_returns.csv')
